...
const StudentDashboard = () => {
  ...
  const fetchDashboardData = async () => {
    ...
    if (enrolledResult.status === 'fulfilled' && enrolledResult.value.data.success) setEnrolledCourses(enrolledResult.value.data.courses || []);
    else setEnrolledError('Could not load your courses.');
    
    if (allResult.status === 'fulfilled' && allResult.value.data.success) setAllCourses(allResult.value.data.courses || []);
  ...
  const getInitials = (name = '') => name.trim().split(' ').slice(0, 2).map(n => n[0]?.toUpperCase()).join('');

  const renderCourseGrid = (coursesList, error) => {
    if (error) return <Alert severity="error" sx={{ mt: 2, background: 'rgba(211, 47, 47, 0.1)', color: '#ffb7b7' }}>{error}</Alert>;
    
    if (coursesList.length === 0) return (
      <Box sx={{ textAlign: 'center', py: 10, background: 'rgba(255,255,255,0.03)', borderRadius: 4, border: '1px dashed rgba(255,255,255,0.1)' }}>
          ...
          return (
            <Grid size={{xs:12, sm:6, md:4, lg:3}} key={course._id} sx={{ display: 'flex' }}>
              <Card sx={{ 
                ...
                transition: '0.3s', '&:hover': { transform: 'translateY(-8px)', borderColor: 'rgba(100, 255, 218, 0.4)' }
              }}>
                <Box sx={{ height: 250, width: '100%', overflow: 'hidden', position: 'relative' }}>
                  {course.thumbnailUrl ? (
                    <CardMedia component="img" image={course.thumbnailUrl} alt={course.title} sx={{ height: '100%', width: '100%', objectFit: 'cover' }} />
                  ) : (
                ...
                <CardContent sx={{ flexGrow: 1, display: 'flex', flexDirection: 'column', gap: 1 }}>
                  ...
                  <Typography variant="subtitle1" sx={{ fontWeight: 700, lineHeight: 1.3, color: '#ffffff', display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>
                    {course.title}
                  </Typography>
                  <Typography variant="body2" sx={{ color: 'rgba(255, 255, 255, 0.7)', flexGrow: 1, display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>
                    {course.subtitle}
                  </Typography>
                ...
                <Box sx={{ p: 2, pt: 0 }}>
                  <Button component={Link} to={`/course/${course._id}`} fullWidth variant={isEnrolled ? 'contained' : 'outlined'} sx={{ borderRadius: 2, textTransform: 'none', fontWeight: 600 }}>
  ...
  return (
    <Container maxWidth="lg" sx={{ py: 8 }}>
      <Typography variant="h4" sx={{ fontWeight: 800, mb: 1, color: '#fff' }}>Student Workspace</Typography>
      <Typography sx={{ color: 'rgba(255, 255, 255, 0.6)', mb: 4 }}>Pick up where you left off, or explore something new.</Typography>
      
      <Tabs value={tabValue} onChange={(_, v) => setTabValue(v)} sx={{ mb: 4, '& .MuiTab-root': { color: 'rgba(255,255,255,0.5)', textTransform: 'none', fontWeight: 600 }, '& .Mui-selected': { color: '#fff !important' } }}>
        <Tab label={`My Courses (${enrolledCourses.length})`} />
        <Tab label="Explore Catalog" />
      </Tabs>

      {tabValue === 0 ? renderCourseGrid(enrolledCourses, enrolledError) : renderCourseGrid(allCourses, catalogError)}
    </Container>
  );
};
