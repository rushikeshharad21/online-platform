...
export const createCourse = async (req, res, next) => {
  try {
    ...
    const newCourse = await Course.create({
      title, subtitle, description, category, price, level,
      curriculum, thumbnailUrl, rating,
...
export const getMyCourses = async (req, res, next) => {
  try {
    ...
    // Count enrolled students per course from StudentProfile
    const courseIds = courses.map(c => c._id);

    const enrollmentCounts = await StudentProfile.aggregate([
...
export const updateCourse = async (req, res, next) => {
  try {
    const {
      title, subtitle, description, category, price,
      level, curriculum, thumbnailUrl, rating, isPublished,
    } = req.body;
